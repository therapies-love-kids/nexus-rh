...
export default function Login() {
    const [selectedDepartamento, setSelectedDepartamento] = useState<any>('');
    const [selectedProfissional, setSelectedProfissional] = useState<any>('');
    const [senha, setSenha] = useState<string>('');
    const [loginStatus, setLoginStatus] = useState<{ type: 'info' | 'success' | 'error'; message: string } | null>(null);
    const navigate = useNavigate();
    const [mostrarSenha, setMostrarSenha] = useState<boolean>(false);

    // Utilize useDepartamentos para obter os departamentos
    const departamentos = useDepartamentos('ativo');

    // Use o departamento selecionado para filtrar os profissionais
    const { profissionais } = useProfissionais('profissionais/fotos', 'ativo', selectedDepartamento || undefined);
                    ...
                    <div className='card-body' onKeyDown={handleKeyDown}>
                        ...
                        <div className="form-control mt-4">
                            ...
                            <select
                                ...
                                onChange={(e) => setSelectedProfissional(Number(e.target.value))} // Certifique-se de converter para number
                                className="select select-bordered w-full"
